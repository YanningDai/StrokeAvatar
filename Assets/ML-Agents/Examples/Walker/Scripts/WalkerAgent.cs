...
public class WalkerAgent: Agent
{
    ...
    protected override void Awake()
    {
        ...
        dirOut = "/Assets/ObserAction/walk73.json";
    ...
    public override void Initialize()
    {
        ...
        observationLogList = jsonDataLog.observationLog;

        dataV = observationLogList[observationLogList.Count - 1][4];
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        step = 1;
        KeyPhaseNum = 0;
        KeyPhaseTotal = (int)Math.Round(dataV / MTargetWalkingSpeed * (keyend - keyini + 1), 0);
    ...
    public float TotalRewardCalculate()
    {
        ...
        footDragPenal = FootDrag();
        walklengthReward = 0.005f;
        walkAbilityReward = footDragPenal + walklengthReward;
        KeyPhase = keyini + (int)Math.Round((double)(keyend - keyini + 1) / KeyPhaseTotal * KeyPhaseNum, 0);

        float datahip = observationLogList[KeyPhase][0] * 0.01f + IniHipHeight;
        var delta1 = (FootHeight(hips) - datahip) / 0.4f;
        keyHipHightReward = (float)Math.Exp(-delta1 * delta1);


        datahip = observationLogList[KeyPhase][1] * 0.3f + IniFootHeightLeft;
        keyHipHightReward += (float)Math.Exp(-delta1 * delta1);
        datahip = observationLogList[KeyPhase][2] * 0.3f + IniFootHeightRight;
    ...
    public float FootHeight(Transform foot)
    {
        ...
        hit = Physics.RaycastAll(foot.position, Vector3.down, 500f, ~(1 << 0));

        for (int c = 0; c < hit.Length; c++)
        {
            if (hit[c].collider.gameObject.CompareTag("ground"))
