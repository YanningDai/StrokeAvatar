...
public class GoalwithImitation : Agent
{
    ...
    {
        Time.fixedDeltaTime = 1.0f / 30f;//物理帧时长
        ...
        //如果训练目标是速度or步长，radom有指定值，否则radom都是false？

        //楼梯：只模仿姿势，速度步长可以忽略？或者步长用
    }
    ...
    public override void OnEpisodeBegin()
    {
        ...
        //确定要模仿的文件
        lengthGoal =
            randomizeWalkLengthEachEpisode ? Random.Range(0.42f, 1.2f) : lengthInput;//应该放一个已知的，1.5096f
        velocityGoal =
            randomizeWalkSpeedEachEpisode ? Random.Range(0.4f, 1.6f) : velocityInput;//1.3828f
        freGoal = velocityGoal / lengthGoal;

        jsonDir = dirPatient;//如果用病人数据，就直接从给定文件里读
    ...
    public void TotalRewardCalculate(ImuData2 imuData2, float iDataset)
    {
        ...
        keyVelocityReward = (float)Math.Exp(- 0.1f * keyVelocityReward);//先省略
